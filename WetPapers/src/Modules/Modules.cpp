...
#pragma region connect camera and imgProModule
	...
	QObject::connect(&cameraModule, &CameraModule::frameCaptured2,
		imgProModule.imageProcessingModule2.get(), &ImageProcessingModuleWetPapers::onFrameCaptured, Qt::DirectConnection);
#pragma endregion

#pragma region connect UIModule and imgProModule
	QObject::connect(imgProModule.imageProcessingModule1.get(), &ImageProcessingModuleWetPapers::imageNGReady, uiModule._wetPapers, &WetPapers::onCameraNGDisplay);
	QObject::connect(imgProModule.imageProcessingModule2.get(), &ImageProcessingModuleWetPapers::imageNGReady, uiModule._wetPapers, &WetPapers::onCameraNGDisplay);
	QObject::connect(uiModule._wetPapers, &WetPapers::shibiekuangChanged, &imgProModule, &ImgProModule::onUpdateImgProContext);
	QObject::connect(uiModule._wetPapers, &WetPapers::shibiekuangChanged, &imgProModule, &ImgProModule::onUpdateImgProContext);
...
#pragma region connect UIModule and ReconnectModule
	// 更新UI界面
	QObject::connect(reconnectModule.monitorCameraAndCardStateThread.get(), &CameraAndCardStateThreadWetPapers::updateCameraLabelState,
		uiModule._wetPapers, &WetPapers::updateCameraLabelState, Qt::QueuedConnection);
	// 相机重连
	QObject::connect(reconnectModule.monitorCameraAndCardStateThread.get(), &CameraAndCardStateThreadWetPapers::buildCamera,
		&cameraModule, &CameraModule::onBuildCamera, Qt::QueuedConnection);
	QObject::connect(reconnectModule.monitorCameraAndCardStateThread.get(), &CameraAndCardStateThreadWetPapers::startMonitor,
		&cameraModule, &CameraModule::onStartCamera, Qt::QueuedConnection);
	// 相机销毁
	QObject::connect(reconnectModule.monitorCameraAndCardStateThread.get(), &CameraAndCardStateThreadWetPapers::destroyCamera,
		&cameraModule, &CameraModule::onDestroyCamera, Qt::QueuedConnection);

	// 运动控制器重连
	QObject::connect(reconnectModule.monitorCameraAndCardStateThread.get(), &CameraAndCardStateThreadWetPapers::buildZmotion,
		&motionControllerModule, &MotionControllerModule::onBuildZmotion, Qt::QueuedConnection);
	// 运动控制器销毁
	QObject::connect(reconnectModule.monitorCameraAndCardStateThread.get(), &CameraAndCardStateThreadWetPapers::destroyZmotion,
...
#pragma region connect UIModule and RuntimeInfoModule
	QObject::connect(runtimeInfoModule.detachUtiltyThread.get(), &DetachUtiltyThread::updateStatisticalInfo,
		uiModule._wetPapers, &WetPapers::onUpdateStatisticalInfoUI, Qt::QueuedConnection);
#pragma endregion
...
#pragma region check config format and exist
	...
	checkFileExistAndFormat<cdm::WetPapersConfig>(globalPath.WetPapersConfigPath, storageContext);
